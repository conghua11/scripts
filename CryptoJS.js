                ...
                var l = o.SHA512 = e.extend({
                    ...
                    _doProcessBlock: function(t, e) {
                        for (var r = this._hash.words, i = r[0], n = r[1], o = r[2], s = r[3], c = r[4], l = r[5], f = r[6], u = r[7], d = i.high, p = i.low, _ = n.high, v = n.low, y = o.high, g = o.low, B = s.high, w = s.low, k = c.high, b = c.low, x = l.high, S = l.low, m = f.high, A = f.low, z = u.high, H = u.low, C = d, R = p, D = _, E = v, M = y, P = g, F = B, W = w, O = k, I = b, U = x, K = S, X = m, L = A, j = z, T = H, N = 0; N < 80; N++) {
                            ...
                            var ft, ut = O & U ^ ~O & X, dt = I & K ^ ~I & L, pt = C & D ^ C & M ^ D & M, _t = R & E ^ R & P ^ E & P, vt = (C >>> 28 | R << 4) ^ (C << 30 | R >>> 2) ^ (C << 25 | R >>> 7), yt = (R >>> 28 | C << 4) ^ (R << 30 | C >>> 2) ^ (R << 25 | C >>> 7), gt = (O >>> 14 | I << 18) ^ (O >>> 18 | I << 14) ^ (O << 23 | I >>> 9), Bt = (I >>> 14 | O << 18) ^ (I >>> 18 | O << 14) ^ (I << 23 | O >>> 9), wt = a[N], kt = wt.high, bt = wt.low, xt = j + gt + ((ft = T + Bt) >>> 0 < T >>> 0 ? 1 : 0), St = yt + _t;
                            j = X,
                            T = L,
                            X = U,
                            L = K,
                ...
                var e = c
                  ...
                  , d = o.RIPEMD160 = n.extend({
                    ...
                    _doProcessBlock: function(t, e) {
                        ...
                        var o, c, d, w, k, b, x, S, m, A, z, H = this._hash.words, C = f.words, R = u.words, D = s.words, E = a.words, M = h.words, P = l.words;
                        b = o = H[0],
                        x = c = H[1],
                        S = d = H[2],
                        m = w = H[3],
...
const utils = creatUtils();
const currentDate = utils.currentDate();
console.log(currentDate)
const hash = utils.md5('yourString');
console.log(hash)


// 假设 `utils` 是你的工具对象，并且已设置为使用 CBC 模式
const customCryptoJS = creatUtils().createCryptoJS();
const secretKey = 'mySecretKey';
const iv = '0000000000000000'
const message = 'myMessage';

// 使用 CBC 模式加密消息
const encrypted = customCryptoJS.AES.encrypt(message, secretKey, {
    iv: iv,
    mode: customCryptoJS.mode.CBC
}).toString();
console.log('加密后的消息:', encrypted);

// 使用 CBC 模式解密消息
const decrypted = customCryptoJS.AES.decrypt(encrypted, secretKey, {
    iv: iv,
    mode: customCryptoJS.mode.CBC
});
const originalText = decrypted.toString(customCryptoJS.enc.Utf8);
console.log('解密后的消息:', originalText);
